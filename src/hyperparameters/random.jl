...
function generate_random_hyperparameters(; 
    ...
    num_img_layers = rand(rng, 3:7)

    @info "Number of image layers: $num_img_layers"
    
    # Core architecture parameters
    pfm_length = rand(rng, 6:15)
    num_base_filters = rand(rng, 256:12:512)
    
    # Generate conv layer filter counts (preserve last layer size for compatibility)
    conv_filter_counts = [rand(rng, 256:32:512) for _ = 1:(num_img_layers-1)]
    push!(conv_filter_counts, 48)  # Fixed final layer size
    ...
    conv_filter_heights = [rand(rng, 2:5) for _ = 1:num_img_layers]
    
    # Pooling configuration (conservative to preserve sequence information)
    base_pool_size = 1
    base_stride = 1
    conv_pool_sizes = [rand(rng, 1:2) for _ = 1:(num_img_layers-1)]
    push!(conv_pool_sizes, 1)  # No pooling on final layer
    
    conv_strides = [rand(rng, 1:2) for _ = 1:(num_img_layers-1)]
    push!(conv_strides, 1)  # No stride on final layer
    
    # Training configuration
    max_pooling_layer = num_img_layers - 1  # Standard configuration
    selected_batch_size = isnothing(batch_size) ? rand(rng, [64, 128, 256]) : batch_size
    ...
    return HyperParameters(
        ...
        pool_base = base_pool_size,
        stride_base = base_stride,
        poolsize = conv_pool_sizes,
        stride = conv_strides,
        pool_lvl_top = max_pooling_layer,
        softmax_strength_img_fil = SOFTMAX_ALPHA,
